...
void  RtcEngineEventHandler::onFacePositionChanged( int ImageWidth, int ImageHeight, agora::rtc::Rectangle* vecRectangle, int* vecDistance, int NumFaces )
{
#if defined(__ANDROID__) || (defined(__APPLE__) && TARGET_OS_IOS)
   TArray<FRectangle> VecRectangle;
   TArray<int> VecDistance;
   for( int i( 0 ); i < NumFaces; i++ )
   {
      VecRectangle.Add( FRectangle( vecRectangle[i].x, vecRectangle[i] .y, vecRectangle[i].width, vecRectangle[i].height ) );
      VecDistance.Add( vecDistance[i] );
   }
   m_functionQueue.push( [=]()
   {
      if( m_eventHandler )
         m_eventHandler->OnFacePositionChangedDelegate.Broadcast( ImageWidth, ImageHeight, VecRectangle, VecDistance, NumFaces );
   } );
#endif
}

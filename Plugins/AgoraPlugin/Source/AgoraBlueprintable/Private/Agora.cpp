...
int UIAgora::SetBool( const FString& Key, bool Bool )
{
	if( !rtcEngine )
		return -1;

	agora::rtc::ue4::AParameter parameter( rtcEngine.Get() );
	return parameter->setBool( TCHAR_TO_ANSI( *Key ), Bool );
}

int UIAgora::SetInt( const FString& Key, int Int )
{
	if( !rtcEngine )
		return -1;

	agora::rtc::ue4::AParameter parameter( rtcEngine.Get() );
	return parameter->setInt( TCHAR_TO_ANSI( *Key ), Int );
}

int UIAgora::SetUInt( const FString& Key, int64 UInt )
{
	if( !rtcEngine )
		return -1;

	agora::rtc::ue4::AParameter parameter( rtcEngine.Get() );
	return parameter->setUInt( TCHAR_TO_ANSI( *Key ), static_cast<unsigned int>( UInt ) );
}

int UIAgora::SetNumber( const FString& Key, float Number )
{
	if( !rtcEngine )
		return -1;

	agora::rtc::ue4::AParameter parameter( rtcEngine.Get() );
	return parameter->setNumber( TCHAR_TO_ANSI( *Key ), Number );
}

int UIAgora::SetString( const FString& Key, const FString& String )
{
	if( !rtcEngine )
		return -1;

	agora::rtc::ue4::AParameter parameter( rtcEngine.Get() );
	return parameter->setString( TCHAR_TO_ANSI( *Key ), TCHAR_TO_ANSI( *String ) );
}

int UIAgora::SetObject( const FString& Key, const FString& Object )
{
	if( !rtcEngine )
		return -1;

	agora::rtc::ue4::AParameter parameter( rtcEngine.Get() );
	return parameter->setObject( TCHAR_TO_ANSI( *Key ), TCHAR_TO_ANSI( *Object ) );
}

int UIAgora::GetBool( const FString& Key, bool& Bool )
{
	if( !rtcEngine )
		return -1;

	agora::rtc::ue4::AParameter parameter( rtcEngine.Get() );
	return parameter->getBool( TCHAR_TO_ANSI( *Key ), Bool );
}

int UIAgora::GetInt( const FString& Key, int& Int )
{
	if( !rtcEngine )
		return -1;

	agora::rtc::ue4::AParameter parameter( rtcEngine.Get() );
	return parameter->getInt( TCHAR_TO_ANSI( *Key ), Int );
}

int UIAgora::GetUInt( const FString& Key, int64& UInt )
{
	if( !rtcEngine )
		return -1;
	...
	agora::rtc::ue4::AParameter parameter( rtcEngine.Get() );
	int ret = parameter->getUInt( TCHAR_TO_ANSI( *Key ), uiValue );

	UInt = uiValue;
	return ret;
}

int UIAgora::GetNumber( const FString& Key, float& Number )
{
	if( !rtcEngine )
		return -1;
	...
	agora::rtc::ue4::AParameter parameter( rtcEngine.Get() );
	int ret = parameter->getNumber( TCHAR_TO_ANSI( *Key ), dValue );

	Number = dValue;
	return ret;
}

int UIAgora::GetString( const FString& Key, FString& String )
{
	if( !rtcEngine )
		return -1;

	agora::util::AString retValue;

	agora::rtc::ue4::AParameter parameter( rtcEngine.Get() );
	int ret = parameter->getString( TCHAR_TO_ANSI( *Key ), retValue );
	if( retValue )
	   String = ANSI_TO_TCHAR( retValue->c_str() );

	return ret;
}

int UIAgora::GetObject( const FString& Key, FString& Object )
{
	if( !rtcEngine )
		return -1;
	...
	agora::rtc::ue4::AParameter parameter( rtcEngine.Get() );
	int ret = parameter->getObject( TCHAR_TO_ANSI( *Key ), retValue );
	if( retValue )
	   Object = ANSI_TO_TCHAR( retValue->c_str() );

	return ret;
}

int UIAgora::GetArray( const FString& Key, FString& Array )
{
	if( !rtcEngine )
		return -1;
	...
	agora::rtc::ue4::AParameter parameter( rtcEngine.Get() );
	int ret = parameter->getArray( TCHAR_TO_ANSI( *Key ), retValue );
	if( retValue )
	   Array = ANSI_TO_TCHAR( retValue->c_str() );

	return ret;
}

int UIAgora::SetParameters( const FString& Parameters )
{
	if( !rtcEngine )
		return -1;
	return rtcEngine->setParameters( TCHAR_TO_ANSI( *Parameters ) );
}

int UIAgora::SetProfile( const FString& Profile, bool Merge )
{
	if( !rtcEngine )
		return -1;

	agora::rtc::ue4::AParameter parameter( rtcEngine.Get() );
	return parameter->setProfile( TCHAR_TO_ANSI( *Profile ), Merge );
}

int UIAgora::ConvertPath( const FString& FilePath, FString& ConvertedPath )
{
	if( !rtcEngine )
		return -1;
	...
	agora::rtc::ue4::AParameter parameter( rtcEngine.Get() );
	int ret = parameter->convertPath( TCHAR_TO_ANSI( *FilePath ), retValue );
	if( retValue )
	ConvertedPath = ANSI_TO_TCHAR( retValue->c_str() );
